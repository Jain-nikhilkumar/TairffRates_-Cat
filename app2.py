    ...
    with col_dl:
        ...
        if st.button("📊 Prepare Full Export"):
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f"Filtered_Tariff_Data_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",
                mime="text/csv",
            )
    ...
    with left:
        ...
        with st.container():
            ...
            st.download_button(
                ...
                data=pd.DataFrame(columns=["HS Code", "Country"]).to_csv(index=False), 
                file_name="core_mapping_template.csv",
                mime="text/csv",
            ...
            with st.status("🔮 Analyzing Schema & Mapping Data...", expanded=True) as status:
                try:
                    ...
                    if ct_key:
                        ...
                        # We use double quotes to handle column names with spaces
                        ...
                        # Logic: Unique HS Mapping
                        enrich_cols_sql = ", ".join([f'm."{c}"' for c in to_fetch if c != "HS Code"])
                        sql = f"""
                            SELECT u.*, {enrich_cols_sql} 
                            FROM upload_data u 
                            LEFT JOIN (SELECT DISTINCT * FROM data) m ON u."{hs_key}" = m."HS Code"
                        """
                    ...
                    csv_data = final_res.to_csv(index=False).encode('utf-8')
                    st.download_button(
                        label="📥 DOWNLOAD ENRICHED DATASET (CSV)",
                        data=csv_data,
                        file_name=f"CORE_Enriched_Report_{datetime.now().strftime('%H%M')}.csv",
                        mime="text/csv",
                        use_container_width=True
                    )
